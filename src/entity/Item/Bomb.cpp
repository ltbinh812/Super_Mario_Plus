...
void Bomb::activate() {
    ...
    active_ = true;
    timer_  = EXPLODE_TIME;
    std::cout << "[Bomb] Countdown started!\n";
}
...
void Bomb::update(float dt) {
    ...
    if (exploded_) {
        ...
        if (!damageEmitted_ && commandQueue) {
            ...
            damageEmitted_ = true;
            std::cout << "[Bomb] Explosion Entity queued at "
...
float Bomb::getRenderOffsetY() const {
...
void Bomb::onInteract(Entity& other) {
...
void Bomb::forceInteract(Entity& other) {
    Player* p = dynamic_cast<Player*>(&other);
    if (p) {
        p->getRuntimeStatsMutable().storedItemSlot = "Boom";
        itemState_ = ItemState::Used;
